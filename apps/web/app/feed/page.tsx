...
const FeedPage = async (): Promise<React.ReactElement> => {
  ...
  const { data, error } = await supabase
    .from('feed_posts')
    .select('id,caption,published_at,saved_try_on_id,saved_try_ons(result_path,garment,fit_physics_note),feed_likes(user_id),feed_comments(id,body,created_at)')
  ...
  const cards = postCards.filter((card): card is NonNullable<typeof card> => card !== null);

  return (
    <section className="panel feed-page" aria-labelledby="feed-title">
      ...
      {cards.length === 0 ? (
        <div className="empty-state">
          <h2>The feed is waiting for its first look.</h2>
          <p>Save a try-on from your wardrobe, then share it with the community.</p>
        </div>
      ) : (
        <div className="feed-list">
          {cards.map(({ post, imageUrl, title, note }) => (
            // The id is the permalink target that ShareButton links to.
            <article className="feed-post" id={`post-${post.id}`} key={post.id}>
              <div className="feed-post-image">
                {imageUrl === null
                  ? <div className="image-fallback">Image unavailable</div>
                  : <img src={imageUrl} alt={`Community try-on for ${title}`} />}
              </div>
              <div className="feed-post-content">
                ...
                {note === null ? null : (
                  <section className="ai-review" aria-labelledby={`review-${post.id}`}>
                    <p className="eyebrow">AI fit review</p>
                    <h3 id={`review-${post.id}`}>{note.summary}</h3>
                    <dl>
                      <div><dt>Stretch</dt><dd>{note.stretch}</dd></div>
                      <div><dt>Structure</dt><dd>{note.structure}</dd></div>
                      <div><dt>Pressure points</dt><dd>{note.pressurePoints?.join(' · ')}</dd></div>
                      <div><dt>Uncertainty</dt><dd>{note.uncertainty}</dd></div>
                    </dl>
                    <p className="review-disclaimer">{note.disclaimer}</p>
                  </section>
                )}
                <PostEngagement
                  postId={post.id}
                  savedTryOnId={post.saved_try_on_id}
                  title={title}
                  likeCount={post.feed_likes.length}
                  liked={post.feed_likes.some((like) => like.user_id === userData.user.id)}
