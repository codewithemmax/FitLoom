...
// Must stay in sync with the API's fileSizeLimitBytes in user-routes.ts.
...
export const PhotoUpload = (): React.ReactElement => {
  ...
  const addPersonalPhoto = (files: FileList | null): void => {
    ...
    if (preview === null) { setMessage('Choose one JPG, PNG, or WebP image under 10 MB.'); return; }
    if (personalPhoto !== null) URL.revokeObjectURL(personalPhoto.url);
  ...
  const addProductPhotos = (files: FileList | null): void => {
    ...
    if (next.length === 0) { setMessage('Choose JPG, PNG, or WebP product images under 10 MB each.'); return; }
  ...
  if (step === 'review' && personalPhoto !== null && productPhotos.length > 0) {
    // Hand over the previews that already exist here rather than creating a
    // second set of object URLs for the same files.
    return <TryOnForm prefillBasePhoto={personalPhoto} prefillGarmentImages={productPhotos} onBack={() => setStep('intake')} />;
  }
